...
var SeedLoader = function SeedReloader(message_obj) {
    ...
    */
    this.seed = function seed(fn) {
        ...
        that.getParsers(function() {
            ...
            var mul_buckets = [];
            for (var ind in message.get('domain_group')) {
                var sub_group = message.get('domain_group')[ind];
            ...
            try {
                pool.bucket_collection.insertMany(mul_buckets, {ordered: false}, function(err, doc) {
                    ...
                    for (var ind in successfull) {
                        var domain_group_id = successfull[ind];
                    ...
                    pool.mongodb_collection.insertMany(inserted_docs, {ordered: false},function initBySeed(err, doc) {
    ...
    this.generateSubLists = function generateSubLists(results, fn) {
        ...
        pool.bot_collection.find({}).toArray(function(err, docs) {
            ...
            //return fn();
            pool.domain_group_collection.insertMany(docs_temp, {ordered: false}, function bulkInsertSublist() {
        ...
        moveSeedCollection(function() {
            try {
                ...
                for (var keys in json) {
                    ...
                    })(obj["_id"], obj["parseFile"], obj["priority"], obj["fetch_interval"], obj["limit_breadth"]);
                }
            } catch (err) {
    ...
    */
    ...
    this.insertSeed = function insertSeed(url, parseFile, priority, fetch_interval, limit_breadth, fn) {
