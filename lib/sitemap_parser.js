...
sitemap.parse = function(url, callback){
	...
	fs.readFile(parent_dir+"/sitemaps/"+(this.url.replace(/\//g,"##")), function(err, file_data) {
	    if(err){
	    	//file not exists then download
				...
				req.on('response', function(res) {
			      ...
			      //write sitemap into cache
			      var stream=fs.createWriteStream(parent_dir+"/sitemaps/"+(this.url.replace(/\//g,"##")));
				  res.pipe(stream);
					console.log();
	    	...
	    	if(gunzip){
		    	//console.log(input,self.url)
		    	var out=zlib.gunzip(file_data,function(err,uncompressed){
		    		if(!check.assigned(uncompressed)){
		    			callback(err,null);
				    	return;
		    		}
	    		...
	    		var file_data=file_data.toString();
	    		try{
		    	xmlParse(body, function(err,file_data){
