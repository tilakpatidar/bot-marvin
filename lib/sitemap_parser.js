        ...
        req.on('response', function(res) {
            ...
            res.on('end', function(err) {
                ...
                if (gzip) {
                    ...
                    var out = zlib.deflate(input, function(err, uncompressed) {
                    ...
                    for (var i = 0; i < d.url.length; i++) {
                        ...
                        if (url_link.details.accepted) {
                            ...
                            if (check.assigned(obj["priority"][0])) {
                                ...
                                obj["priority"] = parseInt(parseFloat(obj["priority"][0] * 100)); //as range of priority is 0.0 to 1.0
                                if (obj["priority"] === 0) {
                                    obj["priority"] = 1; //min priority
                                }
                                if (check.number(obj["priority"])) {
                                    //then convert
                                    obj["priority"] = 101 - (obj["priority"]);
                                } else {
                                    obj["priority"] = 1;
                                }
