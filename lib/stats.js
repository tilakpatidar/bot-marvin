...
var app={
	...
	"indexField":function indexField(collection_name,index_name,fn){
			var d={};
			d[index_name]="text";
	...
	"setSeed":function setSeed(js,fn){
	...
	"getPage":function getpage(url,fn){
		pool.stats.getPage(url,function(err,data){
			return fn(err,data);
		});
	},
	"getBucket":function getBucket(url,fn){
		pool.stats.getBucket(url,function(err,data){
			return fn(err,data);
		});
	},
	"search":function search(query,i,fn){
