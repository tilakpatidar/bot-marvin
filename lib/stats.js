...
var app={
	...
	"getCrawledPages":function(bot_name,i,len,sort_key,sort_type,fn){
			if(bot_name==="master"){
				var d={done:true,"abandoned":{$exists:false},"response":{$exists:true}};
			}
			else {
				var d={"updatedBy":bot_name,done:true,"abandoned":{$exists:false},"response":{$exists:true}};
			}
			var sor={};
	...
	"getFailedPages":function(bot_name,i,len,sort_key,sort_type,fn){
			if(bot_name==="master"){
				var d={done:true,"abandoned":{"$eq":true,$exists:true}};
			}
			else {
				var d={"updatedBy":bot_name,done:true,"abandoned":{"$eq":true,$exists:true}};
			}
			var sor={};
			if(!check.assigned(sort_key)){
				sor['lastModified']=-1;
