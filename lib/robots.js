			...
			app.loadCache(function(){
				for (var i = 0; i < urls.length; i++) {
					...
					(function(req_url){
						...
						dict[req_url_new]=[new Date().getTime(),req_url];
						if(!check.assigned(app.bots[req_url])){
								parser.setUrl(req_url_new+'/robots.txt', function(parser_obj, success) {
									  if(blocked[req_url_new]){
									  	return;//silent return
									  }
									  delete dict[req_url_new];//got response without timeout thus remove from timeout check list
									  if(success) {
...
//for timeout
//dict has key as req_url_new => [time,req_url]
var my_interval=setInterval(function(){
for (var key in dict) {
	var now=new Date().getTime();
	//console.log(key+"    "+now+"   "+(dict[key][0]+config.getConfig("robots_timeout")))
	if(now>=(dict[key][0]+config.getConfig("robots_timeout"))){
		//timeout
		log.put(("Robots.txt timeout for "+dict[key][1]),"error");
	  	var parser_obj={"NO_ROBOTS":true};
	  	addToCache(key,parser_obj,function(){
	  		
	  	});
	  	app.bots[dict[key][1]]=parser_obj;//saving robots obj
		queued+=1;
		if(queued===urls_len){
			return glob_fn(null,app.bots);
		}
		blocked[key]=true;
		delete dict[key];
	}
