...
function touch(file_path, data){
    var st = fs.createWriteStream(file_path);
    st.write(data);
    st.end();
}
//touching files
touch(path + "/config/config_override.json", "{}");
touch(path + "/config/db_config.json", "{}");
touch(path + "/db/pids/active_pids.txt", "");
touch(path + "/db/pids/tikaPID.txt", "");

var dir = path + "/pdf-store-parsed";
...
var req = request({
    uri: "https://archive.apache.org/dist/tika/tika-server-1.11.jar"
});
var done_len = 0;
