...
*/
var Cluster = function(config_obj, pool_obj, trigger_obj, logger_obj){
	...
	*/
	...
	/**
		...
		@param {Config} config
		@param {MongoDb} pool
		@param {Cluster} this - passing current context
		@type Stat
	*/
	var stats_obj = new Stats(config, pool, this);
	...
	*/
	this.closeBot = function closeBot(bot_name,fn){
		that.getBotConfig(bot_name,function(c){
			...
			that.sendTo(c.network_host,c.cluster_port,{"command":"exit","loopback_token":loopback_token},function(status,results){
						...
						if(!check.assigned(js) || (js["output"]==="ack" && check.assigned(js["loopback"]))){
							...
							setTimeout(function(){
								//waiting enough time for the other bot to get killed
									that.sendTo(c.network_host,c.cluster_port,{"command":"isActive"},function(status1,results1){
