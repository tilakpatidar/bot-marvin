...
var app={
	...
	"removeBot":function removeBot(bot_name){
		app.closeBot(bot_name,function closeBot3(closed){
		
		});
	},
	"sendTo":function sendTo(host,port,msg,fn){
	...
	"closeBot":function closeBot(bot_name,fn){
		app.getBotConfig(bot_name,function(c){
			...
			app.sendTo(c.network_host,c.cluster_port,{"command":"exit","loopback_token":loopback_token},function(status,results){
						...
						if(!check.assigned(js) || (js["output"]==="ack" && check.assigned(js["loopback"]))){
							...
							process.bot.stopBot(function stopBot1(st){
									if(st){
...
var stats;
exports.init=function init(p,fn){
	//start the http server
	pool=p;
	stats=new require(parent_dir+'/lib/stats.js')(pool);
	
		stats.activeBots(function activeBots(results){
			msg("Obtained cluster info ","success");
			cluster_info={};
			for (var i = 0; i < results[0].length; i++) {
				cluster_info[results[0][i]["_id"]]=results[0][i];
