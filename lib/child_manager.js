...
function nextFailedBatch(){
	...
	failed_db.serialize(function(){
		...
		failed_db.serialize(function() {
				failed_db.each("SELECT * FROM q WHERE count<"+config.getConfig("retry_times_failed_pages")+" AND status=0 LIMIT 0,100",function(err,row){
					//console.log(row);

					failed_db.run("UPDATE q SET status=1 WHERE id=?",[row.id],function(e,r){
						log.put('Retry in failed queue '+row.url,'info');
					});					
					
					var failed_info = JSON.parse(row.failed_info);
					//bucket_id for failed pages is failed_queue_(row.id)_(count)
					li.push({'_id':row.url,'domain':failed_info['domain'], 'bucket_id':failed_info['bucket_id']+'_'+row.id+'_'+row.count});
...
if(!process.webappOnly){
	...
	setInterval(nextFailedBatch,15000);
