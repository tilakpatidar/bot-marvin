...
function nextFailedBatch(){
	...
	var li = [];
	pool.failed_db.find({"status":0 ,"count":{"$lt":config.getConfig("retry_times_failed_pages")}},{"limit": config.getConfig('failed_queue_size')}).toArray(function(err,docs){
		...
		if(check.assigned(docs)){
			...
			pool.failed_db.update({"_id":{"$in":idss}},{"$set":{"status":1}},function(e,r){
