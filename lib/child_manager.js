...
var ChildManager = function(message_obj) {
    ...
    */
    ...
    var getNextBatch = function getNextBatch(result, batchSize) {
        ...
        var grp = JSON.parse(JSON.stringify(message.get('alloted_domain_groups')));
        var indexx = message.get('alloted_domain_groups').indexOf(prev_domain_grp);
        ...
        pool.bucket_collection.findAndModify({
            ...
            if (check.assigned(object) && check.assigned(object.value)) {
                ...
                prev_domain_grp = object["value"]["domains"];
                //console.log(prev_domain_grp, "prev_domain_grp");
    ...
    */
    ...
    function rss_links_updator(){
        ...
        try{
            pool.rss_feeds.findOne({"nextRefresh":{"$lt": new Date().getTime()}}, function(err, doc){
               ...
               if(check.assigned(err) || !check.assigned(doc)){
                ...
                rss_fetcher_obj.getLinks(doc["_id"],function(li){
                    ...
                    pool.rss_feeds.updateOne({"_id": doc["_id"]},{"$set":{"nextRefresh": new Date().getTime() + 86400000}},function(){
    ...
    */
    this.flushInlinks = function flushInlinks(fn) {
        ...
        pool.mongodb_collection.insertMany(inserted_docs, {ordered: false}, function insertLinksMany(err, doc) {
            //console.log(arguments);
            //console.log(err);
            if (check.assigned(err)) {
