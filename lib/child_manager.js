...
var ChildManager = function(message_obj) {
    ...
    var es_client, es_index, es_type, ES_BATCH_SIZE;
    var ELASTIC_CACHE = [];
    ...
    */
    ...
    var RSS_INTERVAL = config.getConfig("rss_refetch_interval");

    if(config.getConfig("elasticsearch_server")){

        var es_config = JSON.parse(JSON.stringify(config.getConfig("elasticsearch"))); //deep copy
        delete es_config["bulk_batch_size"]; //remove bulk size
        ...
        es_index = config.getConfig('elasticsearch','index');
        es_type = config.getConfig('elasticsearch','type');
        ES_BATCH_SIZE = config.getConfig('elasticsearch', 'bulk_batch_size');
        console.log("BATCH size ", ES_BATCH_SIZE);
    ...
    */
    ...
    var getNextBatch = function getNextBatch(result, batchSize) {
        ...
        var grp = prev_domain_grp.splice(0, 1)[0]; //circular queue
    ...
    */
    ...
    function rss_links_updator() {
        ...
        try {
            ...
            //using findAndModify to dequeue using insertOne for enqueue

            pool.rss_feeds.findAndModify({
                "nextRefresh": {
                    "$lt": new Date().getTime()
                },
                "domains":{
            ...
            , function(err, doc) {
                ...
                if (check.assigned(err) || !check.assigned(doc) || doc.value === null) {
                ...
                rss_fetcher_obj.getRssContent(doc["rss_link"], function(li) {

                    var dict = _.indexBy(li, "link");
                    var md5 = generateMD5(JSON.stringify(li));
                    //console.log(md5);

                    

                    pool.rss_feeds.insertOne({
                        "rss_link": doc["rss_link"],
                        "md5": md5,
                        "page": doc["page"],
                        "nextRefresh": new Date().getTime() + RSS_INTERVAL,
                        "domains": doc["domains"]
                    }, function() {


                        if(doc["md5"] == md5){
                            // if no change in rss content
                            msg("No change in rss "+ doc["rss_link"], "info");
                            return message.get("rss_updator_lock").release();
                        }

                        var l = [];

                        var allow_es_index = config.getConfig("es_rss_index");
                        if(allow_es_index){
                            if(!config.getConfig("elasticsearch_server")){
                                allow_es_index = false;
                            }
                        }

                        _.each(dict, function(item, key){
                            ...
                            if(allow_es_index){
                                ...
                                var source = {};
                                source["tstamp"] = new Date().toISOString();
                                source["url"] = key;
                                source["meta_description"] = item["description"];
                                source["title"] = item["title"];
                                source["feed_title"] = item["meta"]["title"];
                                source["news_type"] = "article";
                                source["author"] = item["author"];
                                source["img"] = item["image"]["url"];
                                source["feed_link_used"] = doc["rss_link"];

                                var id = url.details.nutch_key;
                                var tmp = id.split("/")[0].replace(":http","").split(".");
                                tmp.reverse();

                                source["host"] = tmp.join(".");

                                //console.log(source);
                                //using create : if doc already exists no need to create
                                //rss will be created and it will be updated by crawl cycle
                                ELASTIC_CACHE.push({ create:  { _index: es_index, _type: es_type, _id: id } });
                                ELASTIC_CACHE.push(source);
                        ...
                        if(allow_es_index){
                            elasticFlush(); //flush rss feeds
                        }
                        
                        message.set('inlinks_pool', message.get('inlinks_pool').concat(l));

                        msg("links " + l.length + " obtained from rss " + doc["rss_link"], "success");
                        message.get("rss_updator_lock").release();
    ...
    */
    ...
    function elasticFlush(){

        if(ELASTIC_CACHE.length >= ES_BATCH_SIZE){
                //flush
                var tmp = ELASTIC_CACHE.splice(0,ELASTIC_CACHE.length); //empty the list
                ...
                es_client.bulk({
                    ...
                    if(error){
                        ...
                        msg("updated " + ES_BATCH_SIZE + " docs into elasticsearch index '" + es_index + "'"  , "success");
                    }
                });
    ...
    */
    function elasticIndex(dict){
        ...
        if(config.getConfig("elasticsearch_server") && check.assigned(dict["crawled"]) && dict["crawled"] === true && check.assigned(dict["data"]["_source"]) && check.assigned(dict["data"]["_source"]["id"])){
            try{
                delete dict["crawled"];
                ...
                ELASTIC_CACHE.push({ index:  { _index: es_index, _type: es_type, _id: id } });
                ELASTIC_CACHE.push(source);
