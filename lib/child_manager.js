...
function createChild_for_failed_queue(bucket_links,hash,refresh_label){
	...
	var args=[bucket_links,batchSize,pool.links,botObjs,hash,refresh_label,c[0],c[1],c[2],"failed_queue"];
...
function createChild(bucket_links,hash,refresh_label){
	...
	log.put('Child process started '+botId,"success");
	var c= config.getGlobals();// tuple of config,overriden_config,db_config
	var args=[bucket_links,batchSize,pool.links,botObjs,hash,refresh_label,c[0],c[1],c[2],"normal"];
	//not sending args with child process as char length limitation on bash
...
function startTika(){
	if(config.getConfig("tika")){
			...
			log.put("Tika server forked","success");
...
if(!process.webappOnly){
	...
	var tika_restarter = setInterval(function(){
		if(restarter_locked){
			return;
		}
		...
		if(spawned["tika"]){
			try{
				log.put("Check if tika server alive",'info');
				spawned["tika"].send({"ping":true});
				log.put("Tika server alive",'success');
			}catch(e){
				//could be dead
				//restart
				log.put("Tika server could be dead",'error');
				try{
					spawned["tika"].kill();
				}catch(ee){
	...
	process.my_timers= process.my_timers.concat([a,b,c1,tika_restarter]);
